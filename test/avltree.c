...
int avl_height(AVL_node *tree) {
  if (tree == NULL)
    ...
    return 1 + my_Max(avl_height(tree->left_child), avl_height(tree->right_child));
}
...
AVL_node* leftleft_rotate(AVL_node *k1) {
  ...
  k2->right_child = k1;
  k1->height = 1 + my_Max(avl_height(k1->left_child), avl_height(k1->right_child));
  k2->height = 1 + my_Max(avl_height(k2->left_child), avl_height(k2->right_child));
  return k2;
}
...
AVL_node* insert_avl(AVL_node *tree, int key, int *safe_tag) {
  ...
  /* Count height */
  tree->height = 1 + my_Max(avl_height(tree->left_child), avl_height(tree->right_child));
